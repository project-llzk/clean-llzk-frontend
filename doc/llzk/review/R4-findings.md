...
| R4a-6 | `ofSource_eqs_iff` covers only the *assertion* half of `ConstraintsHoldFlat`; the docstring said the whole thing. The lookup half has no semantic theorem, and the bridge that would give it one (`certified_membership`) is instantiated nowhere with its hypothesis never discharged. The module docstring also cited `ofSource_holds_iff`, which does not exist. | Docstrings corrected. And the bridge is now composed: `values_lt_prime_of_diagnose` discharges the canonicity hypothesis from the compiler's own registry check, and `byteTable_lookup_iff` states `ByteTable.Contains t x ↔ ∃ n ∈ @Bytes's values, fromNat n = x` — the lookup half's semantic theorem, for the table the corpus uses. |
