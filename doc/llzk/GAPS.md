...
1. **The erasure — closed by S24.** `Config.ofCertified` took `CertifiedTable`s
   ...
   catch, and it survived four commits here after being written. The five
   supported checked entry points in `WitnessCheck.lean` — `verify`,
   `compileSourceVerified`, `compile`, `emit`, `emitSource` — now take a
   `CertifiedConfig F`, which holds `CertifiedTable`s (an earlier version of
   this entry counted "seven" by including the two theorems about them);
   `ofCertified` is retired and there is
...
> holds of that input and the typed output reconstructed from the ordered
> `@out{j}` assignment.
...
R8 found that the earlier theorem consumed the output-equality conjunct of G9
only as a premise and concluded `Spec` for Clean's internally recomputed output.
`moduleOutput` now reconstructs the typed result from the ordered emitted
`@out{j}` assignment, and `moduleOutput_eq_of_compile` proves that it equals the
Clean output expression. Both generic soundness theorems and all four concrete
headline instantiations conclude `Spec` for that reconstructed module output.
...
`WitnessSet.agree` remain the checks that do the work at every supported checked
compile.
...
- **`native_decide`** is in the trusted base of every concrete claim this project
  ...
  close — but that file is Clean core, which this branch keeps byte-identical
  after D035's accepted one-file Clean overlay, so removing them needs another
  Clean-side review rather than a backend edit. **That premise is a gate** (R6,
...
- **G9 is per-checked-Source, not per-corpus.** It is a precondition of every
...
- `CanonicalRepr`'s enforcement at every source recognition/compilation entry
  point whose semantics depend on `F`, and that its two laws really do pin `val`
  for a prime field;
...
- that Clean's core after the exact reviewed S29 overlay is byte-identical to
  that overlay — and since R6/S29 both the overlay delta and later identity are
  gates rather than observations.

## What is *not established*, and is not a gap either

One thing R6 looked at and left alone remains, recorded so that a later reviewer
does not spend the same hour deciding it is fine:
...
R6 also recorded a then-accepted member-count boundary that no longer holds.
`ConstraintSet.ofModule` still derives counts from declarations, but the
2026-08-22 frontend audit made `WitnessSet.ofModule` derive witness/output
counts from actual ordered writes and require `memberLayout` to match the
declarations exactly. On the supported both-halves path, an extra, missing,
renamed, reordered, retyped, or visibility-changed member is rejected.
